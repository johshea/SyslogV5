...
def events_fragment():
    ...
    items, total, page, per_page = pagination(q)
    args = request.args.to_dict(flat=True); args.pop("page", None)
    return render_template("events_table.html", events=items, total=total, page=page, per_page=per_page, base_path=url_for('index'),
                           base_qs=urlencode(args), args=request.args)
...
def manage():
    ...
    return render_template("manage.html", total=total, latest=latest, hosts=hosts, retention_days=retention_days, retention_last=retention_last, users=users, active_tab=active_tab)
